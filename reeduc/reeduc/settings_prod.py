...
# Nginx terminates SSL and forwards via HTTP internally.
# Trust the X-Forwarded-Proto header from the reverse proxy.
SECURE_PROXY_SSL_HEADER = ("HTTP_X_FORWARDED_PROTO", "https")

# Nginx handles HTTP→HTTPS redirect, so Django doesn't need to.
SECURE_SSL_REDIRECT = ENABLE_HTTPS

# Secure cookies and HSTS when HTTPS is enabled.
